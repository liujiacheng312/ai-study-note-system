<template>
  <div class="page">
    ...
    <div class="panel config-panel">
      <el-alert
        title="API Key 查询时脱敏显示；留空保存会保留原 Key。停用个人配置后，AI 功能使用管理员全局配置。"
        type="info"
        :closable="false"
        show-icon
      />
      ...
      <el-form :model="form" label-width="120px" class="config-form">
        ...
        <el-form-item label="API Key">
          ...
          <div class="hint">当前状态：{{ currentKeyLabel }}</div>
        </el-form-item>
...
const usingGlobalFallback = ref(true)
...
const statusTagType = computed(() => {
  if (usingGlobalFallback.value) return 'warning'
  return form.enabled === 1 ? 'success' : 'info'
})

const statusText = computed(() => {
  if (usingGlobalFallback.value) return '使用全局默认'
  return form.enabled === 1 ? '个人配置已启用' : '个人配置已停用'
})
...
async function save() {
  ...
  try {
    const payload = { ...form }
    const res = await aiApi.updateConfig(payload)
    form.apiKey = ''
    currentMaskedKey.value = res.data.maskedApiKey || ''
    currentKeyConfigured.value = Boolean(res.data.apiKeyConfigured)
    usingGlobalFallback.value = Boolean(res.data.usingGlobalFallback)
    ElMessage.success('个人 AI API 配置已保存')
  } finally {
    saving.value = false
  }
}
